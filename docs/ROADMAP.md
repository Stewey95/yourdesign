...
- ✅ **Editor & Export Reliability Pass**: Shared text layout and explicit font readiness keep the editor, PNG/JPG/PDF DOM export route, and Safari canvas fallback on one typography contract. Text is explicitly modelled as either free-form (default, hugs intrinsic content until its position-aware logical canvas boundary and then wraps) or bounded (template/saved `textBoxWidth`); authored Enter breaks remain explicit and corner dragging scales the font without changing the mode. Desktop live resize locks the exact current used inline width and centre anchor, projects rotation-aware pointer travel onto the starting text box's corner vector, and scales the text and overlay as one compositor preview. It then removes that layer, reasserts the unchanged anchor, and commits `fontSize` once in the pointerup task before synchronously establishing final text/overlay geometry. This prevents pointer-frame reflow, font-size-based scale explosions, anchor drift, and stale Safari raster clipping while preserving final boundary reconciliation. The fallback also matches editor image containment; transparent PNG, JPG, print-ready PDF, and mobile Safari fallback have downloaded-output regression coverage. Desktop Safari text dragging/resizing has a capture-safe window-listener fallback, a separate selection overlay keeps controls above overlapping content without promoting artwork, and centre snapping is audited for both position and visible guides across every shipped canvas preset. Mobile Fit and preset Fit share a fresh workspace measure/scale/centre transaction, and mobile text selection keeps contextual controls without desktop-style rectangle chrome.
...
- 🔹 **Text Alignment Controls**: Add explicit **Left**, **Centre**, and **Right** controls for the persisted `textAlign` model. Alignment changes how wrapped/manual lines sit inside the text area and remains fully independent from dragging or positioning the object on the canvas. Centre is the current default for new free-form text and for legacy items without a stored value; justify is intentionally excluded *[Recommendation - model landed, controls deferred]*.
