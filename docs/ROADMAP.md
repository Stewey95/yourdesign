...
- ✅ **Editor & Export Reliability Pass**: Shared text layout and explicit font readiness keep the editor, PNG/JPG/PDF DOM export route, and Safari canvas fallback on one typography contract. Text is explicitly modelled as either free-form (default, hugs intrinsic content until its position-aware logical canvas boundary and then wraps) or bounded (template/saved `textBoxWidth`); authored Enter breaks remain explicit and corner dragging scales the font without changing the mode. The fallback also matches editor image containment; transparent PNG, JPG, print-ready PDF, and mobile Safari fallback have downloaded-output regression coverage. Desktop Safari text dragging/resizing has a capture-safe window-listener fallback, a separate selection overlay keeps controls above overlapping content without promoting artwork, and centre snapping is audited for both position and visible guides across every shipped canvas preset. Mobile Fit and preset Fit share a fresh workspace measure/scale/centre transaction.
