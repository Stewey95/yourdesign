...
- ✅ **Editor & Export Reliability Pass**: Shared text layout and explicit font readiness keep the editor, PNG/JPG/PDF DOM export route, and Safari canvas fallback on one typography contract. Text is either free-form (intrinsic content capped by the position/rotation-aware canvas boundary) or intentionally bounded (template/saved `textBoxWidth`); Enter breaks are explicit and alignment stays internal. Desktop corner resize now updates the canonical `fontSize` once per animation frame inside one history transaction, so the live object uses the same deterministic wrapping and geometry that pointerup keeps. There is no separate frozen-width/compositor preview or release-time reconciliation shape. The stable centre anchor is reasserted throughout, the text layer avoids three-dimensional promotion, and one selection observer owns overlay measurement. The same canonical painted text node remains responsible for layout while selected and editing; a transparent caret/input surface and independent overlay cannot change its lines or height. Transparent PNG, JPG, print-ready PDF, mobile Safari fallback, persistence and project restoration retain downloaded/output regression coverage. Desktop Safari gestures keep the capture-safe window fallback, selection controls stay above overlapping artwork without changing layer order, snap guides cover every preset, and mobile keeps its existing Fit, pinch, drag and simplified selection behaviour.
