...
- [`CanvasTextItem.tsx`](../components/editor/CanvasTextItem.tsx): Renders editable text, handles inline focus/caret placement, and auto-calculates text height. `TextDesignItem.textBoxWidth` is optional by design: present means an intentionally bounded wrapping box from a template or saved design; absent means free-form content hugs its intrinsic width until the real logical canvas room around the item's centre is exhausted, then wraps at that boundary while authored newlines remain explicit. Rotated free-form text uses projected horizontal/vertical room. [`TextWordContent.tsx`](../components/editor/TextWordContent.tsx) makes each normal word an atomic inline unit, so WebKit moves it intact to the next line; only a single word wider than the entire containing box receives emergency character breaks. `TextDesignItem.textAlign` is an optional forward-compatible `left | center | right` field that controls lines inside the text area, never object position. New text persists `center`, and legacy/missing values resolve to centre to preserve the pre-field rendering default. The persisted `position` is the text box's centre anchor and is immutable during resize; alignment and wrapping only change content inside that anchored box. Corner resizing changes `fontSize` proportionally and never persists or mutates a wrapping width. Its scale comes from projecting pointer travel onto the starting rendered box's corner vector (the same dimensional geometry used for proportional box resizing), not from dividing travel by `fontSize`. During the live pointer gesture, `EditorPreview` freezes the text root's exact current used width and applies one transient compositor scale to the complete text wrapper; the visible display is the sole non-editing intrinsic grid participant, while the invisible measurement span exists only during editing. The text root is block-grid geometry, eliminating anonymous inline baseline space that could offset visible text from the stored centre. At pointerup the preview layer is removed, the starting position is reasserted, and the final `fontSize` is committed once in the same task; the shared boundary contract then reconciles the widest valid whole-word lines and establishes the complete root/display dimensions synchronously before paint. The editing measurement span, display layer, and zero-padding textarea consume the same text contract so the native caret, text, and selection overlay remain aligned.
- [`SelectionOverlay.tsx`](../components/editor/SelectionOverlay.tsx): A canvas-level interaction layer that measures the selected item, then renders its selection chrome and desktop handles above artwork. It consumes the same transient text-resize scale as the text wrapper, keeping the ring and handles attached without a ResizeObserver/state-render race. Text ResizeObserver measurements update inline geometry and React geometry in the same callback; the resize commit also establishes the final overlay box synchronously before its first paint. Mobile text keeps the overlay geometry for interaction ownership but omits the heavy desktop ring; its contextual toolbar carries the selected state. Image/element mobile rings and all desktop rings remain unchanged. Content wrappers retain their true array layer order, so selection does not visually promote an object.
