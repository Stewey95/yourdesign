...
- `2d-a-2-founder-qa`: **Founder QA Stabilisation Sprint 2D.A.2**: Fixed five reproduced editor defects. (1) *Edge selection*: removed the `overflow-hidden` clip on `.editor-canvas-surface` (`EditorCanvas.tsx`) that silently excluded off-canvas content from both painting and pointer hit-testing, breaking click and Layers-panel selection near canvas edges. (2) *Canvas movement restrictions*: removed the render-time position clamp in `EditorPreview.tsx`'s `canvasItems` memo, the canvas-preset-switch clamp, and the duplicate-offset clamp, and replaced the position-dependent text `maxWidth` formula in `CanvasItem.tsx` with a fixed canvas-width cap, so images, SVG elements, shapes, and text can all be positioned freely beyond every canvas edge without snapping back. (3) *Safari Mobile pinch-to-resize*: added `gesturestart`/`gesturechange`/`gestureend` suppression on the editor workspace (`EditorCanvas.tsx`) — Safari's native WebKit magnify gesture ignores `touch-action` and the `user-scalable=no` viewport meta tag (a known WebKit accessibility override since iOS 10), so it was hijacking two-finger pinch instead of the app's own resize logic; Chrome has no equivalent gesture path and was unaffected. (4) *Gripix colour picker*: rebuilt the popover in `ColorPicker.tsx` as a `react-dom` portal rendered into `document.body` with `position: fixed`, escaping all ancestor `overflow`/scroll clipping; added a draggable header, Escape-to-close, and viewport-clamped placement, while preserving EyeDropper, HEX/RGB, hue, saturation/value, opacity, and swatch controls unchanged. (5) *Desktop resize handles*: verified the prior corner-centring fix (`CornerResizeHandles.tsx`) and added the missing `position: relative` on `CanvasShapeItem.tsx`'s wrapper for consistency with the other three item renderers. Verified with `npm run lint` (0 errors), `npx tsc --noEmit` (0 errors), and `npm run build` (clean production build).
